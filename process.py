...
def get_model_params(model_name: str, tree: ObjectifiedElement) -> dict[str, str]:
    ...
    def get_memory_spec() -> tuple[str, str]:
        ...
        internal_memory = re.sub('\(.+\)',' ', internal_memory)
        internal_memory = ' ' + internal_memory.upper()
        
        # print(internal_memory)

        memory_list = internal_memory.split(',')
        memory_seperated = [
            re.findall('[^\w](\d+)\s*([TGMK]B)?', memory)
            for memory in memory_list
        ]
        # print(memory_seperated)

        for i in range(len(memory_seperated)):
            memory = memory_seperated[i]

            if len(memory) > 2:
                print(f"Incorrect number of elements in internalmemory data spec, {model_name}")
                memory_seperated[i] = ["MISSING", "MISSING"]
                continue
            ...
            for j in range(2):
                try:
                    if not memory[j][0]:
                        ...
                        memory[j] = (memory[j][0], memory[(j + 1) % 2][1])

                    memory[j] = ''.join(memory[j])
        ...
        storages = sorted(storages, key=lambda x: int(x[:-2]) * unit(x[-2:]) if x[:-2].isdigit() else -1)
        storages = ','.join(storages)

        rams = set(memory[1] for memory in memory_seperated)
        rams = sorted(rams, key=lambda x: int(x[:-2]) * unit(x[-2:]) if x[:-2].isdigit() else -1)
        rams = ','.join(rams)
...
def get_processed_data() -> dict[str, dict[str, list[str]]]:
    ...
    with open(filepath, "r") as stream:
        processed_data = yaml.load(stream, Loader=yaml.BaseLoader)
...
if __name__ == "__main__":
    ...
    for model in brands_dir.glob("*/*.html"):
        ...
        if model.stem in processed_models['brands'][brand_name]:
            continue

        # if model.stem in {'C260'}:
        #     continue

        print(brand_name, model.stem)
        tree = get_tree(model)
        model_param = get_model_params(model.name, tree)
        serialize(brand_name, model_param)

        processed_models['brands'][brand_name].append(model.stem)
